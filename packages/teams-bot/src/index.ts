...
import { buildSessionStartedCard } from "./adaptive-cards/session-progress";
import {
  MODEL_OPTIONS,
  DEFAULT_MODEL,
...
async function handleMessage(activity: Activity, env: Env, traceId: string): Promise<void> {
  ...
  const threadRootId = activity.replyToId || activity.id;

  // Check for existing thread session
  if (activity.replyToId) {
    const existingSession = await lookupThreadSession(env, conversationId, threadRootId);
    if (existingSession) {
      const callbackContext: TeamsCallbackContext = {
        source: "teams",
        conversationId,
      ...
      log.warn("thread_session.stale", {
        ...
        conversation_id: conversationId,
        root_activity_id: threadRootId,
      });
      await clearThreadSession(env, conversationId, threadRootId);
    }
  }
  ...
  if (result.needsClarification || !result.repo) {
    ...
    // Store pending message for later retrieval
    const pendingKey = `pending:${conversationId}:${threadRootId}`;
  ...
  // Post session started card
  const progressCard = buildSessionStartedCard(
    repo.fullName,
    result.reasoning,
  ...
  await sendReply(
    ...
    [{ contentType: "application/vnd.microsoft.card.adaptive", content: progressCard }]
  );
}
...
async function handleInvoke(activity: Activity, env: Env, traceId: string): Promise<void> {
  ...
  switch (value.action) {
    case "select_repo": {
      const repoId = value.repoId as string;
      if (repoId) {
    ...
    case "save_preferences": {
      ...
      const reasoningEffort = value.reasoningEffort as string | undefined;
      const userId = getTeamsUserId(activity);
...
async function handleRepoSelection(
  ...
  const pendingKey = `pending:${conversationId}:${threadRootId}`;
  const pendingData = await env.TEAMS_KV.get(pendingKey, "json");
  ...
  const progressCard = buildSessionStartedCard(
    repo.fullName,
    `Selected repository: ${repo.fullName}`,
    sessionResult.sessionId,
    env.WEB_APP_URL
  );
